...
Write-Host "   CLEAN_BEFORE_SYNC: true" -ForegroundColor White
...
try {
    ...
    if ($response.data.Count -gt 0) {
        ...
        foreach ($doc in $response.data) {
            $parentInfo = if ($doc.parentDocumentId) { " (filho de: $($doc.parentDocumentId))" } else { " (raiz)" }
            Write-Host "   - $($doc.title)$parentInfo" -ForegroundColor White
        }
    }
} catch {
...
# Executar sincronizacao com deleção hierárquica
Write-Host "`nExecutando sincronizacao com deleção hierárquica..." -ForegroundColor Yellow
...
Write-Host "`nTeste de deleção hierárquica concluido!" -ForegroundColor Cyan
